...
def show_menu():
    """
    Display the main menu with all available options using Rich tables.
    """
    ...
    encrypt_table = Table(
        show_header=True, header_style="bold magenta", title="Encrypt"
    )
    ...
    decrypt_table = Table(
        show_header=True, header_style="bold magenta", title="Decrypt"
    )
...
def process_choice(choice):
    """
    Process the user's menu choice and execute the corresponding operation.
    
    Args:
        choice (str): The user's selected menu option
        
    Returns:
        bool: True to continue, False to exit the program
    """
    try:
        if choice == "1":
